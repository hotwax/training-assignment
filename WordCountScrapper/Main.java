...
class WebContent {
  ...
  String parseUrl(String url) {
    ...
    try {
      ...
      content = content.replaceAll("[^a-zA-Z\\w]", " "); // Regex to get only text without any symbol or special character
    } catch (IOException e) {
...
public class Main {
  ...
  public static void main(String[] args) {
    try {
      ...
      Map<String, Integer> AllwordsFrequencyMap = new TreeMap<String, Integer>(); // Map to Store Word And their Frequency 
      System.out.println("===========================================");
      for (String url : urls) {
        Map<String, Integer> wordFrequencyMap = new TreeMap<String, Integer>(); // URL wise store Word and their frquency 
        ...
        for (String word : words) {
          ...
          int wordOccurence = webContentObj.countWordOccurence( word, urlContent); // Find the Occurence of the word in URL
          wordFrequencyMap.put(word, wordOccurence); // put word and its frequency in map
        }
        wordFrequencyMap = WebContent.sortByValue(wordFrequencyMap); // Sort Map by frequency of the words in desecending order
        int counter = 0;
        for (Map.Entry m : wordFrequencyMap.entrySet())  // Print top 3 words with highest frequency with the help of counter 
        {  
          if (counter++ < 3) 
          {
            ...
            {
              value += AllwordsFrequencyMap.get(key); // Add the frequency of the words which appears in the multiple URLs
            }
